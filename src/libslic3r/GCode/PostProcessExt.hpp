...
namespace post_gcode {
	...
	class genNozzleMap {
	public:
		genNozzleMap(const std::vector<size_t>& lines_ends, std::vector<NozzleInfo>& nInfo) :m_lines_ends(lines_ends), extuderInfos(nInfo) {};
		std::vector<std::shared_ptr<process_command>>& run() {
			size_t offset = m_lines_ends[pos];
			std::string cmd_str = "";
			cmd_str += _genNozzleMap();
			cmd_str += "\n";
	...
	private:
		std::string  _genNozzleMap() {
			std::string nozzleMapStr;
			nozzleMapStr = ";NozzleColorMap:";
			for (const auto& info : extuderInfos) {
				boost::format nzfmt("T%1% C=0x%2% MID=%3% CID=0x%4%,");
				nzfmt% info.EID% info.Color.substr(1) % info.MID% info.CID.substr(1);
